...
| English (canonical) | Definition | 中文 | Usage |
|---------------------|------------|------|-------|
...
| Data Source | The data view (table/view) this entity or relation maps from | 数据来源 | `## Data Source` / `### Data Source` |
| Data Properties | Explicit list of fields (name, type, PK, index) when not using full mapping | 数据属性 | `### Data Properties` |
| Primary Key | Field that uniquely identifies an instance | 主键 | blockquote `**Primary Key**: \`id\``, table column |
| Display Key | Field used for UI label / search display | 显示属性 | blockquote `**Display Key**: \`name\`` |
...
| Endpoints | Relation endpoints: source entity, target entity, type | 关联定义 | `## Endpoints` / table Source, Target, Type |
| Mapping Rules | How source/target properties map (direct or via view) | 映射规则 | `### Mapping Rules` |
...
| Business Semantics | Human-readable meaning of the relation/entity | 业务语义 | `### Business Semantics` |
| Bound Entity | Entity this action operates on | 绑定实体 | table/section `## Bound Entity` |
