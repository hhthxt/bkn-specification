...
| fragment | 文件类型 `type: fragment`，可包含多个 object/relation/action 的混合片段 |
| data | 文件类型 `type: data`，实例数据文件（建议使用 `.bknd` 扩展名） |
| connection | 文件类型 `type: connection`，可复用的数据源连接定义（可选能力） |
...
下表按 **统一标题层级** 组织，适用于所有文件类型（network / fragment / object / relation / action / data）。
...
| `###` | Data Source | The data view or connection this object maps from | 数据来源 | `### Data Source` |
| `###` | Connection | Connection endpoint and auth reference (for type: connection) | 连接定义 | `### Connection` |
...
| `namespace` | object/relation/action/fragment | 命名空间/包名，用于大规模组织与避免冲突（例如 `platform.k8s`） |
| `owner` | object/relation/action/fragment | 负责人/团队（用于审计与审批路由） |
...
### 单连接文件 (type: connection)（可选）

```yaml
---
type: connection                 # 可复用的数据源连接定义
id: string                       # 连接ID，唯一标识
name: string                     # 连接显示名称
network: string                  # 所属网络ID
---
```

Connection 为**可选能力**，用于多个对象共享同一数据源连接信息。文件中需包含 `### Connection` 小节，声明 endpoint 与 auth 引用；**不得在 BKN 文件中写入明文凭据**，仅使用 `secret_ref` 或环境变量引用。
...
- `connection`：通过可复用的 connection 定义连接外部系统；多个对象可引用同一 connection_id
...
在 fragment / network 文件中，多个对象或关系可各自拥有不同的 tags 和 owner。
...
Connection 为**可选能力**，用于多个对象共享同一数据源连接信息。当多个 Object 或 Relation 指向同一外部系统（如同一数据库、同一 API）时，可定义一个 `type: connection` 文件，在 Data Source 中通过 `connection | {connection_id}` 引用。
...
- `Type`：连接类型（如 `postgres`、`mysql`、`rest`、`grpc` 等），由平台或运行时解析
- `Endpoint`：连接端点（URL、host:port 等），不含凭据
- `Secret Ref`：凭据引用，**不得在 BKN 文件中写入明文密码**；使用环境变量名（如 `DB_PASSWORD`）或平台 Secret ID（如 `secret:db_creds`）

### 引用方式

在 Object 或 Relation 的 `### Data Source` 表格中：

| Type | ID | Name |
|------|-----|------|
| connection | {connection_id} | {display_name} |

多个对象可引用同一 `connection_id`，共享连接定义。若引用的 `connection_id` 不存在，导入或加载时应报错。

### 兼容性

- Connection 为**可选能力**，不使用时无需定义任何 `type: connection` 文件
- 现有网络（仅使用 `data_view` 或 `bknd`）解析与运行行为不变
- 无迁移要求；`connection` 为增量扩展
...
目录名（`objects/`、`relations/`、`actions/`、`risks/`、`connections/`、`data/`）为约定，文件的 `type` 字段为定义类型的权威声明。
