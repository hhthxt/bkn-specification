...
下表按 **统一标题层级** 组织，适用于所有文件类型（network / fragment / entity / relation / action / data）。
...
| `data` | 数据文件 | 承载 entity/relation 的实例数据行（建议 `.bknd`） |
...
`.bknd` 文件使用与 `.bkn` 相同的 Markdown 语法，但正文承载的是实例数据表，而非实体/关系/行动定义。
...
entity: scenario            # 与 relation 二选一
source: PFMEA模板.xlsx      # 可选，数据来源
---
```

- `type` 必须为 `data`
- `entity` 或 `relation` 二选一，用于指向对应 `.bkn` 中定义的 ID
- `network` 建议填写，保持与 schema 文件一致
- `source` 为可选字段，用于记录数据来源

### Body

正文由一个标题（`#` 或 `##`）+ 一个 GFM 表格组成。表头应与目标 entity 的 Data Properties 列名（或 relation 的映射字段）一致。
...
实体的 Data Source 决定其数据是否可写入 `.bknd`：
...
- 当 Entity 的 Data Source 为 `data_view` 时，不应为该实体创建 `.bknd` 文件，数据由外部系统提供
- 当 Entity 的 Data Source 为 `bknd` 时，数据存储在 `.bknd` 文件中，可编辑、可版本化
