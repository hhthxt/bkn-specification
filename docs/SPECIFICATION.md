...
| `namespace` | object/relation/action/fragment/delete | 命名空间/包名，用于大规模组织与避免冲突（例如 `platform.k8s`） |
| `owner` | object/relation/action/fragment/delete | 负责人/团队（用于审计与审批路由） |
...
- `#` - 文档/分组标题（例如网络标题，或 `# Objects` / `# Relations` / `# Actions`）
- `##` - 类型定义（`## Object:` / `## Relation:` / `## Action:`）
...
在 `## Object: pod` 的 `### Logic Properties` 后添加：
