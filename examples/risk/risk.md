...
- **Action** 拥有**运行时/计算属性** `risk`，取值为 **`allow`**、**`not_allow`** 或 **`unknown`**。
- 该属性**不写入 BKN 文件**，可由**内置或用户提供的风险评估函数**根据「当前场景 + 带 `__risk__` tag 的知识（规则实例）」计算得出。
- 三态含义：
  - **`allow`**：存在匹配规则且均允许，执行侧可放行。
  - **`not_allow`**：存在匹配规则且至少一条禁止，执行侧应阻断。
  - **`unknown`**：无规则传入或无规则匹配，信息不足，由执行侧按业务策略决定（如默认放行、人工审批或拒绝）。
- 规范中 Action 的静态 **`risk_level`**（low/medium/high）与动态 **`risk`**（allow/not_allow/unknown）分离：前者用于展示与审批，后者用于执行门控。
...
- 键值对，**`scenario_id` 为可选**。
- 当 context 包含 `scenario_id` 时，仅匹配该场景下的规则。
- 当 context **不包含** `scenario_id`（或传空 `{}`）时，**跳过场景过滤**，所有 `action_id` 匹配的规则均参与判定——适用于不需要场景区分的简单用例。
...
- **每条规则**至少需包含 `action_id` 和 **`allowed`**（bool）。`scenario_id` 可选——不带 `scenario_id` 的规则为全局规则。可选包含 `rule_id`、`reason` 等，供日志与展示。
- **匹配逻辑**：
  1. `action_id` 必须与入参一致；
  2. 若 context 包含 `scenario_id`，则规则的 `scenario_id` 也必须一致；
  3. 若 context 不包含 `scenario_id`，跳过场景过滤，仅按 `action_id` 匹配。
...
**场景**：规则列表中没有当前 scenario + action 的条目，或未传入规则。
...
> - 宽松策略：视 unknown 为 allow（默认放行）
> - 严格策略：视 unknown 为 not_allow（默认拒绝）
> - 审批策略：unknown 时触发人工审批流程

### 4.5 示例五：无场景 — 纯 action 级规则

**场景**：不使用 scenario，直接对某些 action 设置全局禁止/放行规则。

```python
# 全局规则：不带 scenario_id
global_rules = [
    {"action_id": "grant_root_admin", "allowed": False, "reason": "全局禁止提权"},
    {"action_id": "query_sensitive_data", "allowed": True, "reason": "允许但需脱敏"},
]
...
# 混合使用：全局规则 + 场景规则
mixed_rules = [
    {"action_id": "restart_erp", "allowed": True, "reason": "全局默认允许重启"},
    {"scenario_id": "sec_t_01", "action_id": "restart_erp", "allowed": False, "reason": "月末封网禁止"},
]
