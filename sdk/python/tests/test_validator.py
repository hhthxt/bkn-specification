...
from bkn.models import DataProperty, DataTable, Entity
from bkn.validator import validate_data_table, validate_network_data


def _entity(props: list[DataProperty]) -> Entity:
    return Entity(id="test_entity", data_properties=props)


def _table(columns: list[str], rows: list[dict[str, str]]) -> DataTable:
    return DataTable(
        entity_or_relation="test_entity",
        columns=columns,
        rows=rows,
    )
...
class TestColumnMatch:
    ...
    def test_extra_column_reported(self):
        schema = _entity([DataProperty(property="id", primary_key=True)])
...
class TestComparisons:
    def test_gte_ok(self):
        schema = _entity([DataProperty(property="qty", type="int32", constraint=">= 0")])
    ...
    def test_lte_ok(self):
        schema = _entity([DataProperty(property="score", type="float64", constraint="<= 100")])
    ...
    def test_range_ok(self):
        schema = _entity([DataProperty(property="v", type="int32", constraint="range(1,5)")])
        table = _table(["v"], [{"v": "3"}])
        assert validate_data_table(table, schema=schema).ok
...
class TestTypeChecks:
    def test_bool_ok(self):
        schema = _entity([DataProperty(property="flag", type="bool")])
    ...
    def test_numeric_ok(self):
        schema = _entity([DataProperty(property="count", type="int32")])
